# CTF-tools 
## My ctf tools stuff to solve tasks

# OSINT

## Метаданные 

[`Metapicz`](http://metapicz.com/)

[`Exif-viewer`](http://exif-viewer.com/)

[`Iana`](https://www.iana.org/whois)

## Поиск юзернеймов

[`Usersearch`](https://usersearch.org/)

[`SocialCatFish`](https://socialcatfish.com/)

## Другие сайты

[`Dorks`](https://www.exploit-db.com/google-hacking-database)

[`Wayback-Machine`](https://archive.org/web/)

# CRYPTO

## Universal tools 

[`CyberShef`](https://gchq.github.io/CyberChef/)

[`CypherIdent`](https://www.boxentriq.com/code-breaking/cipher-identifier)

## Cyphers

[`PlanetCalc`](https://planetcalc.ru/)

[`BaseConvert`](https://www.dcode.fr/base-n-convert)

[`AphinBreaker`](https://www.dcode.fr/affine-cipher)

[`XORbrutforce`](https://github.com/hellman/xortool)

[`ChastotniyAnalis`](https://guballa.de/substitution-solver)

[`Cryptogram`](https://www.boxentriq.com/code-breaking/cryptogram)

## RSA

[RSACTFTOOL](https://github.com/Ganapati/RsaCtfTool)

[RSsolver](https://www.dcode.fr/rsa-cipher)


# STEGA

[`CyberShef`](https://gchq.github.io/CyberChef/)

[`Binwalk`](https://github.com/ReFirmLabs/binwalk)

[`Exiftool`](https://exiftool.org/)

[`Stegsolve`](https://github.com/Giotino/stegsolve/releases)

[`AperiSolve`](https://aperisolve.fr/)

[`Audacity`](https://www.audacityteam.org/)

# WEB

`SQLmap`

`Robots.txt_Check`

# FORENSIC/REVERSE

[`HexEdit`](https://hexed.it/)

[`Wireshark`](https://www.wireshark.org/)

[`Volatility`](https://github.com/volatilityfoundation/volatility)

[`IDA-FREE`](https://www.hex-rays.com/products/ida/support/download_freeware/)

[`GHIDRA`](https://github.com/NationalSecurityAgency/ghidra)






